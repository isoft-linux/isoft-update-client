                                     ...
                                     GDBusInterfaceInfo *iface)
{
    ...
    for (i = 0; iface->annotations && iface->annotations[i]; i++) {
        if (g_str_equal(iface->annotations[i]->key, 
                        "org.isoftlinux.Update.VendorExtension")) {
                                ...
                                const gchar *path)
{
    ...
    while ((name = g_dir_read_name(dir))) {
         ...
         * private to isoftupdate and point it at the file,
        ...
        if (!symlink) {
            g_warning("Found isoft update vendor extension file %s, but file "
        ...
        if (g_str_has_prefix(symlink, prefix) && 
            ...
            g_warning("Found isoft update vendor extension symlink %s, but it "
