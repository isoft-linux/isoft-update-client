...
#define NAME_TO_CLAIM "org.isoftlinux.Update"
                ...
                gpointer          user_data)
{
    ...
    if (!ensure_directory(UPDATE_UPDATES_CONFDIR, error))
        goto out;

    if (!ensure_directory(DOWNLOAD_DIR, error))
        goto out;
    ...
    openlog("isoft-update-daemon", LOG_PID, LOG_DAEMON);
    syslog(LOG_INFO, "started daemon version %s", PROJECT_VERSION);
    closelog();
    openlog("isoft-update-daemon", 0, LOG_AUTHPRIV);
...
main(int argc, char *argv[])
{
    ...
    if (show_version) {
        g_print("isoft-update-daemon " PROJECT_VERSION "\n");
        ret = 0;
        goto out;
    }
