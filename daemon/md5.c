...
int md5_from_str(const char *str, unsigned char *md5num)
{
...
int md5_from_file(const char *fname, unsigned char *md5num)
{
	struct MD5Context md5c; 
	...
	while (1) {
		if (-1 == (ret = read(fd, databuf, BUFSIZ)))
			return errno;
		if (ret == 0)
			break;
...
int md5_str2num(unsigned char *md5num, const char *md5str)
{
	int i;
	char buff[4] = {0};
	unsigned int  n;

	memset(md5num, 0, MD5_NUM_LEN);	
	if (strlen(md5str) != 32)
		return -1;
